...
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| **No se puede hacer `git push`**                 | El repositorio remoto tiene cambios que todavía no están en el equipo local.                      | Ejecutar `git pull` para actualizar el repositorio y después volver a intentar el `push`.                   |
| **Conflicto al hacer `merge`**                   | Dos ramas modificaron la misma parte de un archivo de manera diferente.                           | Revisar las partes marcadas por Git, decidir qué código conservar y después realizar un nuevo commit.       |
| **Se modificó un archivo por error**             | Se realizaron cambios que no se desean conservar.                                                 | Utilizar herramientas como `git restore` para recuperar la versión registrada del archivo.                  |
| **Se hizo un commit incorrecto**                 | El desarrollador guardó cambios antes de terminar o incluyó archivos que no correspondían.        | Revisar el historial y utilizar `git revert` cuando sea necesario deshacer el commit de forma segura.       |
| **Se olvidó agregar un archivo al commit**       | El archivo no fue incluido en el área de preparación.                                             | Ejecutar `git add` sobre el archivo y realizar otro commit.                                                 |
| **Hay archivos que no deberían estar en Git**    | El proyecto contiene archivos temporales, configuraciones locales u otros elementos innecesarios. | Configurar un archivo `.gitignore` para evitar que esos archivos sean registrados.                          |
| **No se sabe qué cambios se hicieron**           | Se modificaron varios archivos y no se recuerda exactamente qué cambió.                           | Utilizar `git status` y `git diff` para revisar el estado y las diferencias.                                |
| **Se está trabajando en la rama equivocada**     | Se realizaron modificaciones sobre una rama que no correspondía.                                  | Guardar los cambios si es necesario y cambiar a la rama correcta mediante `git switch`.                     |
| **El historial es difícil de entender**          | Se realizaron demasiados commits sin mensajes descriptivos.                                       | Utilizar mensajes claros y revisar el historial con `git log`.                                              |
| **Se necesita guardar temporalmente un trabajo** | Hay cambios sin terminar, pero se necesita cambiar de rama.                                       | Utilizar `git stash` para guardar temporalmente las modificaciones y recuperarlas después.                  |
| **El repositorio local está desactualizado**     | Otros integrantes del equipo realizaron cambios en el repositorio remoto.                         | Ejecutar `git pull` antes de continuar trabajando.                                                          |
| **Se eliminó información accidentalmente**       | Se borraron archivos o cambios que todavía eran necesarios.                                       | Revisar el historial de Git para localizar una versión anterior y recuperar la información correspondiente. |
